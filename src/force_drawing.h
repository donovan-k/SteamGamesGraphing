...
private:
  ...
  void normalizePositions(vector<pair<int, int>> &positions,
                          pair<int, int> &min, pair<int, int> &max);

  // draws the positions of each vertex on a png
  void drawPositions(const vector<pair<int, int>> &positions,
  ...
  pair<float, float> unitVector(const pair<int, int> &pos1,
                                const pair<int, int> pos2, bool unit);
